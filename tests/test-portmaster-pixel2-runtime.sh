...
python3 -m py_compile "$BOOTSTRAP" "$RENDERER" "$UPDATE_ADAPTER"
...
grep -q 'ADAPTER_VERSION="49"' "$BUILDER"
grep -q 'ADAPTER_VERSION = 49' "$UPDATE_ADAPTER"
...
grep -q 'plumos_portmaster_audit.py' "$PORT_LAUNCH"
grep -q -- '--ports-root "$PORTS_ROOT"' "$PORT_LAUNCH"

...
EOF
cat >"$TMP/adbd-watchdog" <<'EOF'
#!/bin/sh
printf '%s\n' "$1" >>"$PLUMOS_TEST_ADBD_WATCHDOG_CALLS"
EOF
...
EOF
chmod +x "$TMP/adbd-control" "$TMP/adbd-watchdog" "$TMP/wifi-recovery" \
    "$TMP/network-control"
...
grep -Fxq stop "$TMP/adbd-watchdog-calls"
...
grep -Fxq start "$TMP/adbd-watchdog-calls"
...
test "$(grep -c '^stop$' "$TMP/adbd-watchdog-calls")" -ge 2

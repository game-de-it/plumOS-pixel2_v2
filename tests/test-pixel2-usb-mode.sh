...
EOF
cat >"$TMP/adbd-monitor" <<'EOF'
#!/bin/sh
printf '%s\n' "$1" >>"$PLUMOS_TEST_ADBD_MONITOR_CALLS"
EOF
...
EOF
chmod +x "$TMP/adbd-control" "$TMP/adbd-monitor" "$TMP/wifi-recovery" \
    "$TMP/network-control"
...
grep -Fxq stop "$TMP/adbd-monitor-calls"
...
grep -Fxq start "$TMP/adbd-monitor-calls"
...
test "$(grep -c '^stop$' "$TMP/adbd-monitor-calls")" -ge 2

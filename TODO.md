...
- [x] shared plumOS AppsをPixel2 componentとして実装する
  ...
  - [ ] PortMaster共通互換レイヤーで未知のAArch64 portを事前監査・安全実行する
    ...
    - 2026-08-27: adapter 42でpure-Python ELF audit、SONAME単位のAvahi/nghttp2投影、
      `execve`/`posix_spawn`境界の環境guard、session identityによる孤児process回収を
      共通化。runtime SONAME symlinkも実機監査対象に含め、Moonlight型とRockbox型の
      host fixtureは合格。strict build、実機deploy、
      画面・入力・音・終了後FE復帰を残す。

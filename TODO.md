...
- [x] standalone emulator binaryをPixel2向けに順次build・実機検証する
  ...
  - [ ] 2026-08-28: standalone Mupen64Plus 2.6.0を追加選択肢として移植する。pinned 6 component、Rice GLES2、Pixel2最終回転、`pixel2_joypad`、FUNCTION終了、plumOS音声経路を実装。`d4ab428`の署名Runtimeを実機へ適用し、起動、論理640x480・4:3、物理480x640回転、ALSA進行、FE復帰、起動前後のRuntime全checksumを機械合格。Riceの可変DBが管理app-layerを書き換える問題もlaunch-private data stagingで解消した。既定`retroarch:parallel_n64`は維持し、物理十字キー・各button、実音質、FUNCTION終了、FE導線からの2回目起動をoperator確認する。[検証記録](docs/validation/2026-08-28-pixel2-mupen64plus-standalone.ja.md)

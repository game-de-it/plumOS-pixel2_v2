...
[English](frontend-feature-audit.md)
...
LCD、物理button、USB Wi-Fi、LAN経由SSH/SFTP/FTP/Samba、各Appsの終了後FE復帰は
実機で確認済みです。host gateは表示・導線・payload・永続化の欠落を継続検出しますが、
実装済みcomponentの画面・操作・音声を自動的に証明するものではありません。release
candidateごとに代表経路の実機確認を繰り返します。

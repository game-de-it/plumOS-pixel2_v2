...
static void pixel2_fb_present_pixels(const uint16_t *pixels)
{
	...
	for (y = 0; y < SCREEN_HEIGHT; y++) {
		...
		uint32_t *dst = (uint32_t *)(pixel2_fb_map +
			(pixel2_fb_draw_yoffset + y) * pixel2_fb_fix.line_length +
			pixel2_fb_var.xoffset * 4);
		unsigned x;
...
static int pixel2_fb_init(void)
{
	...
	if (pixel2_fb_fd < 0 ||
	    ...
	    pixel2_fb_var.xres < SCREEN_WIDTH ||
	    pixel2_fb_var.yres < SCREEN_HEIGHT ||
	    pixel2_fb_fix.line_length < (pixel2_fb_var.xoffset + SCREEN_WIDTH) * 4) {
		return -1;
	}
	...
	if (pixel2_fb_size < (size_t)pixel2_fb_fix.line_length *
	                    (pixel2_fb_var.yoffset + SCREEN_HEIGHT)) {
		return -1;
	}
